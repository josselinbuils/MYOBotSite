...
var actionBlocks = [{
  ...
  'blocks': ['accelerate', 'decelerate', 'auxMotorAccelerate', 'auxMotorSlowDown']
}, {
  'title': 'Actions continues',
  'blocks': ['moveForward', 'moveBackward', 'turnLeft', 'turnRight', 'veerLeft', 'veerRight', 'stop', 'auxMotorFoward', 'auxMotorBackward', 'auxMotorStop']
}];
...
$(function () {
  ...
  $('.block > div:first-child').each(function () {
    if ($(this).parent().attr('data-help')) {
      ...
      $(this).find('.helpBlock').tooltip({
        placement: 'right',
        container: 'body',
        title: $(this).parent().attr('data-help')
      });
    }
  });
  ...
  $(document).on('dragover', '.droppable', function (e) {
    ...
    $(this).children('.block').each(function () {
      if ($(this).attr('id') !== 'marker') {
        var dyBefore = Math.abs(y - $(this).offset().top),
          dyAfter = Math.abs(y - ($(this).offset().top + $(this).outerHeight()));
